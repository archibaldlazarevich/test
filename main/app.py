...
def create_app():
    ...
    def get_client_list():
        """Метод описывающий работу эндпоинта для получения списка всех клиентов в базе данных"""
        clients: List[Client] = db.session.query(Client).all()
        user_list = [c.to_json() for c in clients]
        return jsonify(user_list), 200


    @app.route('/clients/<client_id>', methods=['GET'])
    def get_client_ifo_by_id(client_id: int):
        """Метод описывающий работу эндпоинта для получения информации о клиенте по его id"""
    ...
    def post_new_client():
        """Метод описывающий работу эндпоинта для добавления нового пользователя в базу данных"""
    ...
    def post_new_parking():
        """Метод описывающий работу эндпоинта для добавления нового паркинга в бд"""
        ...
        new_parking = Parking(address= address, opened= opened,
    ...
    def post_new_client_parking():
        """Метод описывающий работу эндпоинта по добавлению клиента, пользующегося паркингом"""
        ...
        if check_parking.opened and check_parking.count_available_places != 0:
            ...
            db.session.commit()
            return (f'Клиент с id {client_id} въехал на парковку {parking_id} '
                    f'в {time_in.strftime('%H:%M:%S')}'), 201
        return f'Данная парковка занята, клиент не смог припарковаться', 404


    @app.route('/client_parkings', methods=['DELETE'])
    def delete_client_parking():
        """Метод описывающий работы эндпоинта по присваиванию клиенту времени выезда с парковки"""
        client_id = request.form.get('client_id')
        parking_id = request.form.get('parking_id')
        time_out = datetime.now()

        check_client: Client = db.session.get(Client, client_id)
        print(check_client, 'afdsfdsfsd')
        ...
        return f'Клиент {client_id} покинул парковку {parking_id} в {time_out.strftime('%H:%M:%S')}', 202
