...
def create_app():
    ...
    def post_new_client_parking():
        ...
        return ("Данная парковка занята, "
                "клиент не смог припарковаться"), 404
    ...
    def delete_client_parking():
        ...
        if check_client.credit_card is None:
            return ("У клиента не привязана"
                    " карта для оплаты"), 404

        check_client_parking: ClientParking = (
            db.session.get(ClientParking, client_id))
        if check_client_parking.time_in > time_out:
            return ("Время выезда клиента меньше,"
                    " чем время заезда"), 404
